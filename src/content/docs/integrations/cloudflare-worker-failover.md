...
The Worker attempts primary for every request. A network failure, five-second timeout, or selected upstream `502`, `503`, or `504` causes that request to receive the static outage page. Every later request tries primary again, so the site returns automatically after the VPS recovers.
...
Cloudflare recommends a **Workers Route** when a Worker runs before an external application server. A Custom Domain is for a Worker that is itself the origin and is not the deployment model here.
...
- fail over only for network errors, the explicit timeout, and `502/503/504`;
...
4. Do not attach `sprey.win/*` yet.

## Test on a separate hostname

Use a temporary hostname such as `failover-test.sprey.win` to validate the full path without changing production:
...
3. In the Worker's **Settings > Domains & Routes**, add `failover-test.sprey.win/*` in the `sprey.win` zone.
...
6. During a controlled maintenance window, make only the test hostname or path return `503`. Do not stop the production WordPress service for this test.
7. Confirm the Worker returns the static page with HTTP `503` and `X-Sprey-Failover: static-outage-page`.
8. Restore primary and confirm the very next request returns WordPress without a DNS change.
...
4. Validate the home page, product pages, assets, cart, checkout, account, and WordPress administration.
5. Review Worker logs and analytics for exceptions, timeouts, fallback responses, and Free-plan usage.
...
Normal WordPress responses have no `X-Sprey-Failover` header. Controlled failover returns `503` and either `static-outage-page` or `fallback-unavailable` in that header.
